...
<Warning>
  ...
  model](https://continue.dev/inception/mercury-coder), or
  [Sweep Next Edit 1.5B](https://ollama.com/Maternion/sweep-next-edit-1.5B)
  configured in
...
- Compatible AI models (Mercury Coder, Instinct, or Sweep Next Edit 1.5B)
...
<Note>
  To use Next Edit, you must have Instinct, Mercury Coder, or Sweep Next Edit
  1.5B configured in your Continue autocomplete model settings. These models
  are specifically designed for next edit predictions. Once a model is loaded,
  reload VS Code to activate it.
  <img
...
- **Sweep Next Edit 1.5B**: Open-source next edit model available in Ollama (`maternion/sweep-next-edit-1.5B`)
